...
c.style[f]=e;f=f.charAt(0).toLowerCase()+f.slice(1);c.style[f]=e},addVendorEventListener:function(b,c,d,a){if(typeof(addEventListener)=="function"){if(c.match(/^webkit/i)){c=c.replace(/^webkit/i,"")
...
}d.save(c);if(c!==this.keysKey){this.addKeyToList(c)}return f}return false},getItem:function(a){if(this.canAddBehavior()){var b=this.element();
b.load(a);var d=b.getAttribute(this.attribute);var c=AC.Storage.item.read(d);delete d;
if(c===null||typeof c==="undefined"){return null}else{if(typeof c==="object"&&"value" in c){return c.value
}else{return c}}}return null},removeItem:function(a){if(this.canAddBehavior()){var b=this.element();
b.load(a);b.removeAttribute(this.attribute);b.save(a);if(a!==this.keysKey){this.removeKeyFromList(a)
}return true}return false},keysKey:"acstkeys",attribute:"content",canAddBehavior:function(){if("addBehavior" in document.body){var a=this.element();
...
document.head.appendChild(this._element)}return this._element},addKeyToList:function(b){var d=this.getItem(this.keysKey);
if(d==null||!("indexOf" in d)){d=[]}if(d.indexOf(b)==-1){d.push(b)}for(i=0;i<d.length;
i++){if(this.getItem(d[i])===null){var c=d.length;var a=d.slice(0);d=d.splice(0,i).concat(a.splice(i+1,c-i+1));
delete c,a}}this.setItem(this.keysKey,d);return true},removeKeyFromList:function(c){var e=this.getItem(this.keysKey);
if(e==null){e=[]}if(e.indexOf(c)!==-1){var b=e.indexOf(c);var d=e.length;var a=e.slice(0);
e=e.splice(0,b).concat(a.splice(b+1,d-b+1));delete b,d,a}this.setItem(this.keysKey,e);
return true}},cookie:{setItem:function(b,c,d){if(AC.Detector.hasCookies()){if(typeof d=="undefined"||d===null){d=this.options.daysBeforeExpiring
